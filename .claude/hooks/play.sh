...
# Attempt to source .env for credentials (script dir, current dir, or $HOME)
...
for env_file in "${ENV_CANDIDATES[@]}"; do
  if [[ -f "$env_file" ]]; then
    echo "Loading .env from: $env_file" >&2
    set +u
    set -a
...
echo "OPENAI_KEY set: $([ -n "$OPENAI_KEY" ] && echo 'YES' || echo 'NO')" >&2
echo "ELEVENLABS_API_KEY set: $([ -n "$ELEVENLABS_API_KEY" ] && echo 'YES' || echo 'NO')" >&2
echo "ELEVENLABS_VOICE_ID: $ELEVENLABS_VOICE_ID" >&2
...
echo "HOOK_DATA received: $HOOK_DATA" >&2
...
echo "PROMPT extracted: $PROMPT" >&2
...
if [[ -n "$ELEVENLABS_API_KEY" && -n "$ELEVENLABS_VOICE_ID" ]]; then
  ...
  echo "ElevenLabs credentials not found. Message saved to tmp.txt but not played." >&2
  echo "To enable TTS, set ELEVENLABS_API_KEY and ELEVENLABS_VOICE_ID in your .env file." >&2
fi

echo "Hook completed at $(date)" >&2
echo "========================================" >&2
